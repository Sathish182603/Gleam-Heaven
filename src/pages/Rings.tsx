...
const Rings = () => {
  ...
  const addRingCollection = async () => {
    if (!user || !isAdmin) {
      toast({
        title: "Access Denied",
        description: "You must be logged in as an admin to add ring collections",
        variant: "destructive",
      });
      return;
    }
    
    // Sample ring collection - 5 gold + 5 silver
    const ringCollection = [
      // Gold Rings
      {
        name: "Royal Gold Diamond Ring",
        description: "Elegant 18k gold ring featuring a brilliant cut diamond centerpiece with intricate detailing and traditional craftsmanship",
      ...
      },
      {
        ...
        description: "Classic men's gold signet ring with engraved family crest design and polished finish",
        category: "rings" as const,
        metal_type: "gold" as const,
        weight_grams: 8.7,
      ...
      },
      {
        ...
        description: "Stunning gold engagement ring with solitaire setting and brilliant cut diamond",
        category: "rings" as const,
        metal_type: "gold" as const,
        weight_grams: 3.2,
      ...
      {
        name: "Gold Band Ring",
        description: "Simple yet elegant gold band ring perfect for wedding or everyday wear",
        category: "rings" as const,
        metal_type: "gold" as const,
        weight_grams: 5.1,
      ...
      },
      {
        ...
        description: "Bold and beautiful gold statement ring with geometric patterns and contemporary design",
        category: "rings" as const,
        metal_type: "gold" as const,
        weight_grams: 6.8,
      ...
      // Silver Rings
      {
        name: "Sterling Silver Rose Ring",
        description: "Exquisite sterling silver ring featuring a detailed rose design with oxidized finish",
        category: "rings" as const,
        metal_type: "silver" as const,
        weight_grams: 5.2,
        image_url: "https://images.unsplash.com/photo-1603561591411-07134e71a2a9?w=400",
        is_featured: false
      },
      {
        name: "Antique Silver Band Ring",
        description: "Vintage-inspired silver band ring with intricate floral engravings and aged patina",
        category: "rings" as const,
        metal_type: "silver" as const,
        weight_grams: 6.3,
        image_url: "https://images.unsplash.com/photo-1605100804763-247f67b3557e?w=400",
      ...
      },
      {
        ...
        description: "Traditional Celtic design silver ring with intricate knotwork and symbolic patterns",
        category: "rings" as const,
        metal_type: "silver" as const,
        weight_grams: 4.9,
        image_url: "https://images.unsplash.com/photo-1515562141207-7a88fb7ce338?w=400",
      ...
      },
      {
        ...
        description: "Delicate silver promise ring with heart motif and romantic engraving",
        category: "rings" as const,
        metal_type: "silver" as const,
        weight_grams: 3.1,
      ...
      },
      {
        ...
        description: "Set of three stackable silver rings with different textures and finishes",
        category: "rings" as const,
        metal_type: "silver" as const,
        weight_grams: 7.8,
    ...
    try {
      ...
      const productsToInsert = ringCollection.map(product => ({
        ...product,
      ...
      toast({
        title: "🎉 Ring Collection Added!",
        description: `Successfully added ${ringCollection.length} rings (5 gold + 5 silver) to your collection`,
        duration: 6000,
      });
      
      await fetchProducts();
    } catch (error: any) {
      console.error('Error adding ring collection:', error);
      toast({
        title: "Error",
        description: error.message || "Failed to add ring collection",
        variant: "destructive",
      });
    }
  };
  ...
  const fetchReviews = async () => {
    ...
    if (data && !error) {
      const reviewsByProduct = data.reduce((acc, review) => {
        ...
        // Create a simplified review object
        ...
        acc[review.product_id].push(reviewData);
        return acc;
      ...
      <div className="container mx-auto px-4 pt-24 pb-8">
        <div className="text-center mb-8">
          ...
          <p className={`text-lg ${theme === 'gold' ? 'text-gold-text/80' : 'text-slate-600'} max-w-2xl mx-auto`}>
            Discover our exquisite collection of handcrafted rings in gold and silver
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mb-8">
          ...
          {isAdmin && products.length === 0 && (
            <Button
              onClick={addRingCollection}
              className="bg-green-600 hover:bg-green-700 text-white flex items-center gap-2"
            >
              <Plus className="h-4 w-4" />
              Add Ring Collection (5 Gold + 5 Silver)
            </Button>
          )}
        </div>
              ...
              >
                <CardContent className="p-6">
                  ...
                  <div className="space-y-2">
                    <h3 className={`font-semibold ${theme === 'gold' ? 'text-gold-text' : 'text-slate-800'}`}>{product.name}</h3>
                    <p className={`text-sm ${theme === 'gold' ? 'text-gold-text/70' : 'text-slate-600'}`}>{product.description}</p>
                    ...
                    <div className={`pt-4 border-t ${theme === 'gold' ? 'border-gold/20' : 'border-slate-200'}`}>
                      <div className={`text-sm ${theme === 'gold' ? 'text-gold-text/70' : 'text-slate-500'} mb-2`}>
                      ...
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <div className={`text-xs ${theme === 'gold' ? 'text-gold-text/50' : 'text-slate-400'}`}>
        ...
        {filteredProducts.length === 0 && (
          <div className="text-center py-12">
            <p className={theme === 'gold' ? 'text-gold-text/70' : 'text-slate-500'}>No rings found for the selected filter.</p>
            {isAdmin && products.length === 0 && (
              <div className="mt-6">
                <Button
                  onClick={addRingCollection}
                ...
                <p className={`text-sm mt-2 ${theme === 'gold' ? 'text-gold-text/60' : 'text-slate-400'}`}>
                  Add a sample ring collection to get started
                </p>
              </div>
            )}
          </div>
